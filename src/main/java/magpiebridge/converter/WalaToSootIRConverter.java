...
public class WalaToSootIRConverter {
  ...
  public Iterator<IClass> iterateWalaJavaSourceClasses() {
  ...
  public ArrayList<SootClass> convertOnlyClass(Iterator<IClass> walaClass) {
    ArrayList<SootClass> res = new ArrayList<>();
    Iterator<IClass> it = walaClass;
    while (it.hasNext()) {
      IClass klass = it.next();
      JavaClass walaJavaClass = (JavaClass) klass;
      SootClass sootJavaClass = this.classConverter.convertClass(walaJavaClass);
      res.add(sootJavaClass);
    }
    return res;
  }
}
